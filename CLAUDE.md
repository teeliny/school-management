...
- **A `Subject` with `isGroup=true` is never itself assignable or scoreable** — only its `childSubjects` are (PRD §3.3; the group's own grade is a computed weighted average via `SubjectGroupWeight`, done by the worker at report time). `GET /subjects` returns group subjects with children nested, not flattened, so any UI or logic that lists subjects for teacher assignment or score entry must flatten `isGroup` subjects into their `childSubjects` itself — see the `selectableSubjects` `flatMap` in `staff-assignment-form.tsx`, `gradebook/page.tsx` and `timetable-slot-form.tsx` for the pattern to copy. `ScoreEntryService.enter` and `TimetableSlotService.create`/`update` also reject a write against a group `subjectId` outright as a backstop, even under Admin override. This bit us three times (each picker shipped listing the group directly) before the backstops went in.
- **`ClassSubject` assigns a subject to a whole `ClassLevelCategory` (e.g. `NURSERY` covers both "Nursery 1" and "Nursery 2" `ClassLevel`s at once) — `ClassSubjectLevelStatus` is the sparse override that lets one specific `ClassLevel` opt out of that category-wide assignment** (same disable-only, "absence of a row means active" shape as its per-term sibling `ClassSubjectTermStatus`; see both models' comments in `schema.prisma`). **Every place that resolves "which subjects apply to this class" has to check it separately — there is no single choke point.** Known checkers today: `ScoreEntryService.enter`, `StudentSubjectEnrollmentService` (both auto-enroll paths and explicit `enroll()`), `SubjectService.findAll` (via an optional `classLevelId` query param — used by the gradebook/subject-comment-panel pickers and `staff-assignment-form.tsx`'s eligible-class-arms filter), `BroadsheetService.build`'s subject columns, and the worker's `resolveRequiredSubjects`/`resolveSubjectsForClassArm` (class timetable *and* exam timetable generation). This list was assembled the hard way — the feature initially shipped covering only the first two, and broadsheet/timetable/exam-timetable were a follow-up fix once someone asked "does disabling a subject actually keep it out of the timetable and reports?" and the answer was no. If you add a new feature that lists, schedules, or reports on "the subjects for class X," check `ClassSubjectLevelStatus` for that class's `classLevelId` too, or add it to this list as a known gap.
