...
- **Component architecture is Atomic Design**, under `apps/web/src/components/`:
  ...
  - `molecules/` — small compositions of atoms/Radix primitives: `FormField` (`Label` + `Input`), `Select`, `DropdownMenu`, `AlertDialog`, `ThemeToggle` (Radix `ToggleGroup`), `Card`/`CardHeader` (the bordered panel used everywhere content is grouped), `Letterhead` (the eyebrow + serif `h1` + double-rule that opens every page).
