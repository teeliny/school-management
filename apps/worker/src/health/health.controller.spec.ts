...
describe("HealthController", () => {
  it("returns the health response as-is when all checks are ok", async () => {
    const service = { check: jest.fn().mockResolvedValue({ status: "ok", service: "worker", checks: [] }) } as unknown as HealthService;
    const controller = new HealthController(service);
    await expect(controller.check()).resolves.toEqual({ status: "ok", service: "worker", checks: [] });
  });
  ...
  it("throws a 503 HttpException carrying the full body when degraded", async () => {
    const degraded = {
      status: "degraded",
      service: "worker",
      checks: [{ name: "database", status: "error", detail: "boom" }],
    };
