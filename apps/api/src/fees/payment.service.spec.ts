...
describe("PaymentService.recordCash (CASH now requires Super-Admin approval, same as a manual bank transfer)", () => {
  ...
  it("creates a PENDING_APPROVAL CASH payment without touching the invoice, receipt, or queue", async () => {
    const result = await service.recordCash({ invoiceId: "invoice-1", amount: 5000 }, BURSAR);

    expect(prisma.payment.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ method: "CASH", status: "PENDING_APPROVAL", amount: 5000 }) }),
    );
  ...
  it("does not notify guardians at submission time — that now happens on approval", async () => {
    await service.recordCash({ invoiceId: "invoice-1", amount: 5000 }, BURSAR);

    expect(notifications.notify).not.toHaveBeenCalled();
  });

  it("rejects recording cash when the invoice has no outstanding balance", async () => {
    prisma.invoice.findUniqueOrThrow.mockResolvedValue(buildInvoice({ payments: [{ amount: 5000, status: "SUCCESSFUL" }] }));

    await expect(service.recordCash({ invoiceId: "invoice-1", amount: 100 }, BURSAR)).rejects.toThrow(/no outstanding balance/);
...
describe("PaymentService.approvePayment / rejectPayment (PRD FR7.3b, extended to CASH)", () => {
  let prisma: ReturnType<typeof buildPrismaMock>;
  let queue: ReturnType<typeof buildQueueMock>;
  let service: PaymentService;
  let notifications: { notify: jest.Mock };
  ...
  it("approves a PENDING_APPROVAL manual transfer: updates the payment, recomputes the invoice, creates a receipt, enqueues generation", async () => {
    const result = await service.approvePayment("payment-abcdef12", "super-1");
  ...
  it("also approves a PENDING_APPROVAL CASH payment", async () => {
  ...
  it("notifies every guardian of the student that a payment was received, on approval", async () => {
    await service.approvePayment("payment-abcdef12", "super-1");
  ...
  it("rejects approving a payment that isn't PENDING_APPROVAL", async () => {
    ...
    await expect(service.approvePayment("payment-abcdef12", "super-1")).rejects.toThrow(/PENDING_APPROVAL/);
    expect(prisma.__tx.payment.update).not.toHaveBeenCalled();
  });

  it.each(["GATEWAY_CARD", "GATEWAY_TRANSFER", "GATEWAY_USSD", "GATEWAY_RESERVED_ACCOUNT"])(
    "rejects approving a %s payment — not a staff-entered method",
    async (method) => {
      prisma.payment.findUniqueOrThrow.mockResolvedValue(buildPendingManualPayment({ method }));
  ...
  it("rejects a PENDING_APPROVAL manual transfer with the given reason, leaving the invoice untouched", async () => {
    const result = await service.rejectPayment("payment-abcdef12", "super-1", "Amount does not match proof");
  ...
  it("also rejects a PENDING_APPROVAL CASH payment", async () => {
    prisma.payment.findUniqueOrThrow.mockResolvedValue(buildPendingManualPayment({ method: "CASH" }));

    const result = await service.rejectPayment("payment-abcdef12", "super-1", "Never actually received");
  ...
  it("rejects rejecting a payment that isn't PENDING_APPROVAL", async () => {
    ...
    await expect(service.rejectPayment("payment-abcdef12", "super-1", "reason")).rejects.toThrow(/PENDING_APPROVAL/);
    expect(prisma.payment.update).not.toHaveBeenCalled();
  });
});
