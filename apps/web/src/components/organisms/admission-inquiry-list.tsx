...
interface AdmissionInquiryItem {
  id: string;
...
export function AdmissionInquiryList({ canManage }: { canManage: boolean }) {
  ...
  const {
    ...
    queryFn: () => apiFetch<AdmissionInquiryItem[]>("/admission-inquiries", { auth: true }),
  });
  const error = queryError instanceof ApiError ? queryError.message : queryError ? "Failed to load inquiries" : null;

  async function markReviewed(id: string) {
    await apiFetch(`/admission-inquiries/${id}`, { method: "PATCH", auth: true });
  ...
  if (inquiries.length === 0) return <p className="text-sm text-muted">No admission inquiries yet.</p>;
      ...
      <table className="w-full text-left text-[12.5px]">
        <thead>
          <tr className="border-b border-border text-muted">
            ...
            <th className="py-2 pr-4 text-[10px] font-medium uppercase tracking-wide">Parent</th>
          ...
          {inquiries.map((inquiry) => (
            <tr key={inquiry.id} className="border-b border-border/60 align-top last:border-none">
              ...
              <td className="py-2.5 pr-4">
                <div className="font-medium">{inquiry.parentFullName}</div>
                <div className="text-muted">{inquiry.parentEmail}</div>
                {inquiry.parentPhone && <div className="font-mono text-muted">{inquiry.parentPhone}</div>}
              </td>
              <td className="py-2.5 pr-4">{inquiry.studentFullName ?? "—"}</td>
              <td className="py-2.5 pr-4">{inquiry.desiredEntryClass}</td>
