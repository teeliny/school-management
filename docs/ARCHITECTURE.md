...
- **HTTP alternative to step E**: `POST /setup/super-admin` (`apps/api/src/identity/setup/setup.ts`) does the identical work via the same `SetupService`, for infra that can deploy the app but can't exec a one-off shell command against it (e.g. some PaaS setups). It's deliberately unauthenticated — no account exists yet for this deployment to authenticate as — so it substitutes two other checks for a JWT guard: the proprietor email must appear in the infra-set `SETUP_ALLOWED_EMAILS` env var (checked before any read or write happens), and the route hard-disables itself (403) the moment any Super-Admin is active, unlike the CLI script which stays re-runnable indefinitely. Leaving `SETUP_ALLOWED_EMAILS` unset disables the endpoint entirely; the CLI script remains the default path and is unaffected either way.
